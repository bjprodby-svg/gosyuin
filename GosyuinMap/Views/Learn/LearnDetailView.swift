...
struct LearnDetailView: View {
    ...
    // MARK: - Chapter Progress Indicator

    private func chapterProgress(current: Int, total: Int) -> some View {
        HStack(spacing: DS.Spacing.sm) {
            ...
            HStack(spacing: 4) {
                ForEach(1...total, id: \.self) { step in
                    Circle()
                        .fill(step <= current ? article.color : Color.progressEmpty)
    ...
    private var typesContent: some View {
        VStack(spacing: DS.Spacing.xxl) {
            ...
            typeCard(category: .jinja, name: "Jinja (Shrine)", japanese: "神社",
            ...
            typeCard(category: .tera, name: "Tera / Ji (Temple)", japanese: "寺",
            ...
            typeCard(category: .jingu, name: "Jingu (Grand Shrine)", japanese: "神宮",
            ...
            typeCard(category: .taisha, name: "Taisha (Grand Shrine)", japanese: "大社",
            ...
            typeCard(category: .tenmangu, name: "Tenmangu", japanese: "天満宮",
            ...
            typeCard(category: .inari, name: "Inari Shrine", japanese: "稲荷神社",
            ...
            typeCard(category: .hachimangu, name: "Hachimangu", japanese: "八幡宮",
    ...
    private var gosyuinContent: some View {
        ...
        return VStack(spacing: DS.Spacing.xxl) {
            ...
            noteCard(icon: "lightbulb.fill", color: article.color, title: "Tips",
                     text: "Shrine and temple goshuin can go in the same book. Don't put tourist stamps or stickers in your goshuincho. Use a protective cover if possible. The book reads right-to-left, like traditional Japanese books.")
        }
    }
    ...
    private var omikujiContent: some View {
        ...
        return VStack(spacing: DS.Spacing.xxl) {
            ...
            VStack(alignment: .leading, spacing: DS.Spacing.md) {
                HStack(spacing: DS.Spacing.sm) {
                    IconBadge(icon: "list.number", size: 28, color: article.color, filled: true)
    ...
    private func fortuneRank(_ kanji: String, _ english: String, _ color: Color, _ width: Double) -> some View {
        HStack(spacing: DS.Spacing.md) {
            ...
            ProgressBar(progress: width, color: color, height: 8)
    ...
    private var mannersContent: some View {
        VStack(spacing: DS.Spacing.xxl) {
            ...
            noteCard(icon: "exclamationmark.triangle.fill", color: .vermillion, title: "Common Mistakes to Avoid",
                     text: "Clapping at temples (Shinto only!), walking in the center of the approach path, skipping temizu purification, treating goshuin as tourist stamps, lighting incense from another person's stick, wearing shoes in restricted areas, and being loud or rowdy — these are active places of worship.")

            mannerSection(icon: "calendar", title: "Seasonal Tips", color: article.color, items: [
                "New Year: Extremely crowded — expect long queues",
                "Cherry Blossom (Mar-Apr): Beautiful but crowded at scenic locations",
                "Obon (August): Buddhist period for honoring ancestors",
                "Shichi-Go-San (Nov 15): Children's festival — expect families in kimono",
            ])
        }
    }
    ...
    private func mannerSection(icon: String, title: String, color: Color, items: [String]) -> some View {
        VStack(alignment: .leading, spacing: DS.Spacing.md) {
            HStack(spacing: DS.Spacing.sm) {
                IconBadge(icon: icon, size: 28, color: color, filled: true)
                Text(title)
                    .font(.subheadline.bold())
            }
    ...
    /// Type card with Canvas-drawn category icon
    private func typeCard(
        category: ShrineCategory, name: String, japanese: String,
        ...
        VStack(alignment: .leading, spacing: DS.Spacing.md) {
            HStack(spacing: DS.Spacing.md) {
                ZStack {
                    ...
                    CategoryIconView(category: category, size: 28, color: category.color)
                }
            ...
            HStack(spacing: DS.Spacing.xs) {
                ...
                Text(examples).font(.caption).foregroundStyle(Color.captionText)
            }
        }
        .cardStyle()
    ...
    private func illustratedStep(
        number: Int, total: Int, title: String, subtitle: String,
        illustration: AnyView?, description: String, delay: Double
    ) -> some View {
        VStack(spacing: DS.Spacing.md) {
            chapterProgress(current: number, total: total)
    ...
    private func noteCard(icon: String, color: Color, title: String, text: String) -> some View {
        HStack(alignment: .top, spacing: DS.Spacing.md) {
            ...
            VStack(alignment: .leading, spacing: DS.Spacing.xs) {
                ...
                Text(text).font(.caption).foregroundStyle(Color.subtitleText)
                    .fixedSize(horizontal: false, vertical: true)
            }
        }
